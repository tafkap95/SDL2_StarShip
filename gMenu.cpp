...
// Create first Button
    gButton m_button1(this->s_context.m_renderer,l_rect, "./data/Button1.bmp","./data/Button2.bmp",1);
...
// Create second Button
    ...
    gButton m_button2(this->s_context.m_renderer,l_rect, "./data/Exit1.bmp","./data/Exit2.bmp",0);
...
void gMenu::set_continue(char c)
{
    this->cont = c;
}
...
int gMenu::ControlButtonEvents(SDL_MouseButtonEvent pButton)
{
    bool l_ret;
    ...
    for(unsigned int i=0; i<=list_buttons.size() - 1; i++)
    {
        ...
        l_ret = this->isInRectangle2(pButton, c_button->box);

        if (l_ret==true)
        {
            return c_button->get_Action();
...
bool gMenu::isInRectangle2(SDL_MouseButtonEvent pbutton, SDL_Rect prect)
{
    int x3;
    int y3;
    bool ret = false;
    ...
    if ((pbutton.x >= prect.x && pbutton.x <= x3) && (pbutton.y >= prect.y && pbutton.y <= y3) )
    {
        ret = true;
    }
